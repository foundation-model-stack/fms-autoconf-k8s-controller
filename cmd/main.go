...
func main() {
	...
	var watchLabelKey, watchLabelValue string
	var doneLabelKey, doneLabelValue string
	...
	flag.StringVar(&namespaces, "namespaces", "", "If set, these wll be the namespaces that the controller "+
	...
	if waitingForAdoRequestIDLabel == "" {
		setupLog.Error(fmt.Errorf(
			"--waiting-for-ado-request-id-label cannot be empty"),
			"Invalid args",
		)
		os.Exit(1)
	}
	...
	if (len(urlAdo) > 0) == (len(pathWrapperScript) > 0) {
		setupLog.Error(fmt.Errorf(
			"--path-wrapper-script and --url-ado are mutually exclusive and there must be exactly one set"),
			"Invalid args",
		)
		os.Exit(1)
	}

	if !enableAppWrapper && !enablePyTorchJob {
		setupLog.Error(fmt.Errorf(
			"Both enable-appwrapper and enable-pytorchjob are set to false."),
	...
	if enableAppWrapper {
		byObject[&awv1beta2.AppWrapper{}] = cache.ByObject{
			Label: labels.SelectorFromSet(labels.Set{watchLabelKey: watchLabelValue}),
		}
	}

	if enablePyTorchJob {
		byObject[&kubeflowv1.PyTorchJob{}] = cache.ByObject{
			Label: labels.SelectorFromSet(labels.Set{watchLabelKey: watchLabelValue}),
		}
	}
	...
	mgrOptions := ctrl.Options{
		...
		LeaderElectionID:       "f9b9a831.codeflare.dev",
	...
	// VV: Restrict manager to certain namespaces
	if len(namespaces) > 0 {
